...
using ProyectoAnalisis.UI.Models;
...
namespace ProyectoAnalisis.UI.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(Model.Usuarios usuario)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = usuario.Correo, Email = usuario.Correo };
                var result = await UserManager.CreateAsync(user, usuario.Contraseña);
                if (result.Succeeded)
                {
                    
                    Business.GestorDeUsuarios gestorDeUsuarios = new Business.GestorDeUsuarios();
                    gestorDeUsuarios.RegistrarunUsuario(usuario);
